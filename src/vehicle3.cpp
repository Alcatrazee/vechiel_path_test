...
int main(int argc, char **argv)
{
  ...
  ros::Publisher path_pub = nh.advertise<nav_msgs::Path>("robot3_path", 10);
  ros::Rate rate(1);
  ...
  while (ros::ok())
  {
    if (client.call(path_req))
    {
      ...
      if (path_req.request.start_point.position.x <= -4.5)
      {
        path_req.request.start_point.position.x += 0.05;
      }
      else
      {
        path_req.request.start_point.position.x -= 0.05;
